...
function noise(x, y, t) {
    ...
    let w1 = sin(0.2 * y + 1.5 * t + 2.8 + 
                 2.3 * sin(0.5 * x + -1.2 * t + 0.5));
    // Use configurable noise time scale
    let timeFactor = sin(t * config.noiseTimeScale) * 0.5;
    return w0 + w1 + timeFactor;
}
...
const config = {
    ...
    walkRadius: 50,
    noiseScale: 2,
    noiseTimeScale: 0.5, // New parameter for noise animation speed
    spiderRadius: 100,
    movementSpeed: 35,
    particleSize: 1,
    lineColor: '#ffffff',
...
function spawn() {
    ...
    function drawLine(x0, y0, x1, y1, t) {
        const points = [];
        many(100, (i) => {
            i = (i + 1) / 100;
            let px = lerp(x0, x1, i);
            let py = lerp(y0, y1, i);
            let k = noise(px / 5 + x0, py / 5 + y0, t) * config.noiseScale;
            points.push(new THREE.Vector3(px + k, py + k, 0));
        });
        const geometry = new THREE.BufferGeometry().setFromPoints(points);
        const line = new THREE.Line(geometry, lineMaterial);
...
gui.add(config, 'walkRadius', 10, 200, 1).name('Walk Radius').onChange(restartAnimation);
gui.add(config, 'noiseScale', 0, 10, 0.1).name('Line Noise').onChange(restartAnimation);
gui.add(config, 'noiseTimeScale', 0.1, 2, 0.1).name('Noise Time Scale').onChange(() => {}); // No restart needed
gui.add(config, 'spiderRadius', 50, 200, 1).name('Spider Radius').onChange(restartAnimation);
gui.add(config, 'movementSpeed', 10, 100, 1).name('Movement Speed').onChange(restartAnimation);
gui.add(config, 'particleSize', 0.5, 5, 0.1).name('Particle Size').onChange(restartAnimation);
gui.addColor(config, 'lineColor').name('Line Color').onChange(restartAnimation);
...
function animate() {
    ...
    renderer.setClearColor(0x000000);
    renderer.clear();
    const t = clock.getElapsedTime(); // Use elapsed time from THREE.js Clock
    spiders.forEach(spider => spider.tick(t));
    renderer.render(scene, camera);
    requestAnimationFrame(animate);
}
...
// function noise(x, y, t) {
//     // Modified noise function with continuous time factor
//     let w0 = sin(0.3 * x + 1.4 * t + 2.0 + 
//                  2.5 * sin(0.4 * y + -1.3 * t + 1.0));
//     let w1 = sin(0.2 * y + 1.5 * t + 2.8 + 
//                  2.3 * sin(0.5 * x + -1.2 * t + 0.5));
//     // Add time-based oscillation for animated movement
//     let timeFactor = sin(t * 0.5) * 0.5; // Continuous oscillation
//     return w0 + w1 + timeFactor;
// }
...
//     renderer.setClearColor(0x000000);
//     renderer.clear();
//     t /= 1000; // Convert to seconds for smoother animation
//     spiders.forEach(spider => spider.tick(t));
//     renderer.render(scene, camera);
//     requestAnimationFrame(animate);
// }
